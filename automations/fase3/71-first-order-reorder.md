...
### Status
**LIVE dal 2026-07-22.** Flow `UrwVp4`, attivato da Andrea nel builder dopo review completa (design R4zvd6, doppio flow filter, split ItemNames, UTM, mittenti). Da qui in poi: MAI delete+recreate — modifiche solo nel builder o via PATCH flow-actions (da testare).

#### Status precedente (montaggio)
**RIMONTATO COME DRAFT IN KLAVIYO via API il 2026-07-22 con copy v4.1 (fix accenti+mittenti v4.3).** Flow ID **`UrwVp4`** ("71 - First-Order Reorder Reminder [API DRAFT v4]"), status `draft`. I draft precedenti (`Xi3wcm` v2, `TvgBJ9` v4.2 con entità HTML) e i loro template sono stati ELIMINATI. Template HTML in libreria (design standard R4zvd6): A1 RqHcUu, A2 TKwSGc, B1 W6kytU, B2 RMQuuV, C1 UKFcTg (Klaviyo li clona nel flow con ID propri). Mittenti già impostati via API: "Flaminia di Paleocomplex" (Email 1) e "Lorenzo Zarone" (Email 2 + VitD), entrambi info@paleocomplex.com (reply-to incluso). Da rifinire nel builder prima di attivare: A/B subject varianti B/C + deep-link riordino.
...
### Changelog
- **v4.8 (2026-07-22)**: flow ATTIVATO da Andrea (live). Chiuso il ciclo montaggio.
- **v4.7 (2026-07-22)**: routing dei 3 split allineato alla sintassi canonica dell'account. Lo SKU del doc e' concettuale: Klaviyo NON espone lo SKU come property filtrabile (sta annidato in $extra.Items), e anche i flow prodotto live 24-37 montati nel builder filtrano su **ItemNames** (list contains). Split riscritti con `property ItemNames, filter list contains-any [nomi]` (stessa sintassi del flow 24 live); corretto anche il precedente `ProductName` string-in che rischiava di non matchare. Mappa SKU↔nome 1:1 verificata sugli eventi: routing identico al design del doc. Nuovo flow `UrwVp4`, eliminato `VE2wwF`.
- **v4.6 (2026-07-22)**: aggiunto al profile_filter di flusso il secondo gruppo AND esplicito **"Placed Order zero times since starting this flow"** (timeframe `flow-start`), richiesto da Andrea (non aggiungibile a mano nel builder). Ora il filtro e' doppio: Placed Order = 1 alltime AND Placed Order = 0 da inizio flow. Nuovo flow `VE2wwF`, eliminato `XXSWf8`.
- **v4.5 (2026-07-22)**: attivato UTM tracking (`add_tracking_params: true`) su tutte le 5 email — ricreato solo il flow riusando i template in libreria (nuovo flow `XXSWf8`, eliminato `VdrJP7`). Verificato read-back: utm=True ovunque.
- **v4.4 (2026-07-22)**: template rifatti col **design standard R4zvd6** (header Shop/Blog/Supporto, saluto "Ciao {{ first_name }}", firma, footer unsubscribe) riusando build_html della skill montaggio-email-klaviyo — la v4.3 aveva un HTML custom minimale, sbagliato. Firma Flaminia ("A presto / Flaminia / Customer Care Paleocomplex") sulle Email 1. Pulsante CTA: "Vai al nostro sito" (E1), "Riordina ora" (E2), "Riordina la tua Vitamina D" (VitD, deep-link prodotto). Nuovo flow `VdrJP7`, eliminato `QVDaWY`. Creata skill `montaggio-flow-klaviyo` con le regole.
- **v4.3 (2026-07-22)**: fix da review Andrea sul montaggio v4.2. (1) **Accenti**: i template usavano entità HTML che rendevano male (è→È e simili) — riscritti con caratteri UTF-8 reali, verificato zero entità residue e È solo a inizio frase; corretti anche oggetti/preview che usavano l'apostrofo ("e' finito"→"è finito"). (2) **Mittenti**: senza `from_email` l'API forzava il sender default "Paleocomplex" — ora impostati via API i mittenti reali presi dai flow live ("Flaminia di Paleocomplex" / "Lorenzo Zarone", info@paleocomplex.com). (3) Fix refuso "dànno"→"danno". Nuovo flow `QVDaWY`, eliminato `TvgBJ9`.
- **v4.2 (2026-07-22)**: rimontato in Klaviyo via API con il copy v4.1. Nuovo flow `TvgBJ9` (draft), eliminati il vecchio `Xi3wcm` e i 5 template v2. Struttura invariata (3 conditional-split annidati su ProductName, profile_filter Placed Order=1, delay 25/5, 50/8, 120). Nei template: variabile reale `{{ event.ProductNames.0 }}`, pagina supporto, blocco risparmio PROMO3/PROMO6+fedelta'+spedizione/pagamenti in tutte le Email 2 e VitD. Restano i to-do builder: mittenti, A/B subject, deep-link riordino.
