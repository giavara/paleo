**Versione:** 1.2
**Ultimo aggiornamento:** 2026-04-15
**Status:** Bozza v1 — in attesa review Andrea

# Flow 3: Welcome Unghie e Capelli

## Chi entra in questo flow

Tutti i nuovi contatti che si iscrivono tramite il **lead magnet "Unghie e Capelli Deboli"**:

- **Landing page sponsorizzata sui social:** `paleocomplex.com/unghie-capelli-deboli/`
- Eventuali form dedicati sul sito che puntano allo stesso report

Cosa hanno richiesto:
- Il report gratuito su capelli deboli, unghie fragili e le cause alla base
- Consenso alle comunicazioni commerciali

Lo sconto BENVENUTO €10 NON è stato promesso nella landing. Viene dato come regalo di benvenuto a sorpresa (stessa logica del Welcome Kit Benessere per i contatti social).

**Chi sono queste persone:**
Prevalentemente donne 35-55 (ma non solo), frustrate da capelli che cadono, unghie che si spezzano, pelle che perde tono. Hanno già provato soluzioni (dermatologi, integratori generici, cosmetici) senza risultati duraturi. Cercano una soluzione che agisca dall'interno.

**Tema dominante:** beauty dall'interno, collagene, capelli, unghie, pelle.
**Prodotti protagonisti:** Youth, Jeunesse.
**Prodotti complementari:** multivitaminici come base per potenziare i risultati.

## Configurazione Klaviyo

**Trigger:** Added to List (lista "Lead Magnet Unghie Capelli")
**Mittente:** Lorenzo Zarone (tutte le email del flow)
**Filtri profilo:**
- Has not been in this flow before

**Uscita dal flow:**
- Se il contatto effettua un acquisto durante il flow → esce e entra nel **Post Purchase New Customer (flow 6)**
- Se completa il flow senza acquistare → entra nell'**Authority Flow (flow 4)**

**Relazione con Authority Flow:**
Authority parte DOPO il completamento di questo welcome (sequenziale, NON in parallelo).

**Note tecniche:**
- Codice BENVENUTO: €10 fissi, solo primo acquisto, valido su tutti i prodotti
- Il report va consegnato come link diretto al PDF

---

## EMAIL 1 — Delivery report + sconto benvenuto (immediata)

### Oggetto (3 varianti A/B)

- A: Ecco il tuo report + un regalo di benvenuto
- B: La verità su capelli e unghie deboli è dentro questo report
- C: Il primo passo per capelli e unghie più forti

### Preview text (3 varianti)

- A: Scarica il report e scopri il tuo regalo di €10.
- B: Le vere cause di capelli deboli e unghie fragili.
- C: Il report che hai richiesto è pronto.

### Corpo email

Ciao [NOME]

Da oggi fai parte del mondo Paleocomplex.

Ecco il report che hai richiesto su capelli deboli e unghie fragili.

**[CTA: Scarica il tuo report gratuito]**
(link al PDF)

All'interno troverai le cause reali di questi problemi e perché la maggior parte delle soluzioni che hai provato finora non ha funzionato. Spoiler: il problema non è quasi mai "esterno".

Come regalo di benvenuto, ho anche un **codice sconto di €10 sul tuo primo acquisto**: **BENVENUTO**.

Il codice è valido su tutti i prodotti dello store, senza limiti di spesa.

**[CTA: Vai allo store e usa il tuo sconto](https://paleocomplex.com)**

**Tra un paio di giorni** voglio raccontarti perché capelli e unghie si indeboliscono davvero. Spoiler: lo shampoo, lo smalto e le creme stanno lavorando sul livello sbagliato. Il vero problema è interno, e ha a che fare con qualcosa che il tuo corpo smette di produrre dopo i 30 anni.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 2 — Perché capelli e unghie si indeboliscono (+2 giorni)

### Oggetto (3 varianti A/B)

- A: Perché i tuoi capelli cadono (non è quello che pensi)
- B: La vera causa di unghie fragili e capelli deboli
- C: Il collagene che perdi ogni giorno e non lo sai

### Preview text (3 varianti)

- A: Non è lo shampoo. Non è lo stress. È qualcosa di più profondo.
- B: Dopo i 30 anni il tuo corpo produce sempre meno collagene.
- C: La scienza dietro capelli, unghie e pelle che si indeboliscono.

### Corpo email

Ciao [NOME]

Eccomi con quello che ti avevo promesso. Hai mai pensato a cosa hanno in comune capelli, unghie e pelle?

Sono tutti fatti di collagene. Il collagene è la proteina strutturale più abbondante del tuo corpo. È quello che tiene insieme i tessuti, dà elasticità alla pelle, forza alle unghie e struttura ai capelli.

Il problema è che **dopo i 25-30 anni la produzione naturale di collagene diminuisce di circa l'1-1,5% all'anno**. A 50 anni ne produci quasi la metà di quando ne avevi 20.

Ecco perché i capelli diventano più sottili. Le unghie si spezzano. La pelle perde elasticità.

Shampoo rinforzanti, smalti indurenti e creme antirughe lavorano sulla superficie. Ma il problema è interno: il tuo corpo ha bisogno di materia prima per ricostruire il collagene che perde ogni giorno.

E non basta prendere un integratore di collagene qualsiasi. Servono tre cose:

**1. Collagene in quantità sufficiente.** La maggior parte degli integratori contiene 2-3 grammi. La ricerca indica che servono almeno 5-10 grammi al giorno per avere un effetto misurabile.

**2. I cofattori giusti.** Il corpo non può sintetizzare collagene senza vitamina C, zinco e biotina. Senza questi cofattori, il collagene che assumi viene sprecato.

**3. Protezione dallo stress ossidativo.** I radicali liberi degradano il collagene più velocemente di quanto il corpo riesca a ricostruirlo. Servono antiossidanti che proteggano quello che stai ricostruendo.

La buona notizia? Agire su tutti e tre questi fronti è possibile. E più semplice di quanto pensi.

Ne parliamo nelle prossime email.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 3 — Chi è Lorenzo + brand story angolo beauty (+4 giorni)

### Oggetto (3 varianti A/B)

- A: Perché ho creato integratori diversi da tutti gli altri
- B: La mia storia (e perché la bellezza parte dall'interno)
- C: Non sono un brand di cosmetici. Ecco perché.

### Preview text (3 varianti)

- A: Non è nato come business. È nato come necessità.
- B: Integratori con dosaggi reali, non simbolici.
- C: La frustrazione che mi ha portato a creare Paleocomplex.

### Corpo email

Ciao [NOME]

Mi chiamo Lorenzo Zarone. Sono laureato in Scienze Motorie con specializzazione in Nutrizione e Nutraceutica, e sono il fondatore di Paleocomplex.

Ti racconto in breve perché questo brand esiste.

Anni fa, studiando nutrizione e integrazione, mi sono reso conto di una cosa che mi ha frustrato profondamente: **la maggior parte degli integratori sul mercato non funziona**. Dosaggi ridicoli, forme economiche che il corpo fatica ad assorbire, etichette che sembrano impressionanti ma che nascondono il nulla.

Il mondo del collagene è l'esempio perfetto. Prodotti da 2-3 grammi spacciati per miracolosi. Formule senza i cofattori necessari. Pubblicità che promettono "pelle giovane in 7 giorni" con dosaggi che non possono fare nulla.

Ho creato Paleocomplex per offrire l'alternativa: **integratori con dosaggi che possono fare davvero la differenza, in forme che il corpo può assorbire, con tutto quello che serve e niente di superfluo.**

Ogni ingrediente ha una ragione precisa. Ogni dosaggio è studiato per avere un effetto reale. E l'etichetta ti dice tutto, senza nascondere nulla.

Se vuoi saperne di più, sul nostro sito trovi tutto: ingredienti, dosaggi, spiegazioni, confronti. Trasparenza totale.

**[CTA: Scopri i nostri prodotti](https://paleocomplex.com)**

**Tra qualche giorno** voglio mostrarti i due prodotti specifici che abbiamo creato per capelli, unghie e pelle. Te li racconterò onestamente, dirò la differenza tra loro, e ti farò leggere cosa dicono le persone che li usano da mesi. Anche le donne con la mia stessa frustrazione di partenza.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 4 — Youth e Jeunesse: la soluzione diretta + social proof (+7 giorni)

### Oggetto (3 varianti A/B)

- A: La soluzione per capelli, unghie e pelle più forti
- B: 6.000 mg di collagene al giorno (non 2.000 come gli altri)
- C: Ecco cosa dicono chi ha risolto il tuo stesso problema

### Preview text (3 varianti)

- A: Due prodotti. Dosaggi reali. Risultati visibili.
- B: Collagene ad alto dosaggio + 12 nutrienti sinergici.
- C: Capelli più corposi, unghie più forti, pelle più luminosa.

### Corpo email

Ciao [NOME]

Eccomi con i due prodotti che ti avevo promesso. Te li racconto onestamente, come avevo detto.

Nella scorsa email ti ho spiegato perché capelli, unghie e pelle si indeboliscono: il collagene cala, e servono dosaggi reali, cofattori giusti e protezione antiossidante per ricostruirlo.

Abbiamo creato due prodotti che fanno esattamente questo.

**Youth:** 6.000 mg di collagene bovino idrolizzato al giorno con 12 nutrienti sinergici: acido ialuronico, glucosamina, condroitina, MSM, vitamina C, zinco, biotina, NAC e aminoacidi precursori del collagene. Un unico prodotto che copre capelli, unghie, pelle e articolazioni a dosaggi seri. **€59 per 30 giorni, circa €1,97 al giorno.**

**Jeunesse:** per chi vuole il massimo. **12.000 mg di collagene bovino grass-fed** (il doppio di Youth), più antiossidanti cutanei avanzati: astaxantina, estratto di corteccia di pino e centella asiatica per stimolare la produzione naturale di collagene. Glucosamina di origine vegetale (adatto a chi è allergico ai crostacei). **€99 per 50 giorni, circa €1,98 al giorno.**

Entrambi si prendono la sera, preferibilmente 1-1,5 ore prima di dormire. La glicina nel collagene favorisce il rilassamento, e la fase rigenerativa del corpo avviene durante il sonno profondo.

Ecco cosa dicono chi li usa:

**"Dopo soli 10 giorni di assunzione la mia pelle è diventata più morbida e liscia, i capelli più corposi e robusti. Un integratore davvero favoloso."** Manuela, Youth

**"Ho 40 anni e vi assicuro che neanche a 20 l'avevo così elastica e bella. Capelli folti e lucenti e ottimo risultato anche alle ciglia."** Alice B., Jeunesse

**"Unghie durissime, capelli più folti. Non ne posso fare a meno."** Anna, Jeunesse

Non sai quale scegliere tra i due? La regola è semplice: Youth se vuoi un ottimo prodotto completo. Jeunesse se vuoi il massimo assoluto in termini di collagene e anti-aging cutaneo.

**[CTA: Scopri Youth](https://paleocomplex.com/prodotto/youth/)**

**[CTA: Scopri Jeunesse](https://paleocomplex.com/prodotto/jeunesse/)**

Ricorda che hai ancora il tuo sconto di benvenuto: **codice BENVENUTO** per €10 sul primo acquisto.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 5 — Il multivitaminico come base che potenzia i risultati (+9 giorni)

### Oggetto (3 varianti A/B)

- A: Il segreto per far funzionare meglio il collagene
- B: Perché il collagene da solo non basta
- C: L'errore che fa chi prende solo il collagene

### Preview text (3 varianti)

- A: Senza le vitamine giuste, il collagene non si ricostruisce.
- B: Il pezzo mancante nel protocollo capelli e pelle.
- C: La base che potenzia tutto il resto.

### Corpo email

Ciao [NOME]

Nella scorsa email ti ho parlato di Youth e Jeunesse per capelli, unghie e pelle. Oggi voglio completare il quadro.

Il collagene è il "costruttore". Ma il costruttore ha bisogno di un terreno fertile per lavorare.

Il tuo corpo utilizza **vitamina C, zinco, rame, biotina, vitamina A e antiossidanti** per sintetizzare e proteggere il collagene. Se questi nutrienti scarseggiano, anche il miglior collagene del mondo rende la metà.

Youth e Jeunesse contengono già vitamina C, zinco e biotina. Ma un multivitaminico completo porta il supporto a un livello superiore: vitamine del gruppo B per il metabolismo cellulare, vitamina E per la protezione delle membrane, selenio, minerali traccia e antiossidanti avanzati.

Per questo molti dei nostri clienti abbinano il collagene a un multivitaminico. Il protocollo completo è semplice:

**Al mattino** prendi il multivitaminico dopo colazione.
**La sera** prendi Youth o Jeunesse prima di dormire.

Due gesti al giorno, e il tuo corpo ha tutto quello che serve per ricostruire dall'interno.

Quale multivitaminico scegliere? Dipende dalle tue esigenze. Abbiamo preparato una guida che ti aiuta a trovare quello giusto in base alla tua età e ai tuoi obiettivi.

**[CTA: Vai alla guida alla scelta](https://paleocomplex.com/guida-scelta)**

Una nostra cliente l'ha detto meglio di me:

**"Ho speso tanti soldi, anche da dermatologi, ma nessuno riusciva a fermare la caduta. Da quando uso Paleocomplex la ricrescita è aumentata, prima li tagliavo sempre molto corti perché erano fini e deboli, ora sono più forti."** Cinzia G., Paleocomplex

Il codice **BENVENUTO** è ancora attivo: **€10 di sconto sul primo acquisto**.

**[CTA: Vai allo store](https://paleocomplex.com)**

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 6 — Ultima occasione (solo per chi NON ha acquistato) (+12 giorni)

### Condizione

**Split PRIMA dell'email:** Has Placed Order at least 1 time since starting this flow?
- **SÌ** → Esce dal flow (è già nel Post Purchase)
- **NO** → Riceve questa email

### Oggetto (3 varianti A/B)

- A: Il tuo sconto di €10 sta per scadere
- B: Un'ultima cosa prima di salutarti
- C: La scelta più semplice per capelli e unghie più forti

### Preview text (3 varianti)

- A: Il codice BENVENUTO è ancora attivo, ma non per molto.
- B: Questa è l'ultima email sul tuo sconto di benvenuto.
- C: Due prodotti, un protocollo semplice, risultati visibili.

### Corpo email

Ciao [NOME]

Questa è l'ultima volta che ti parlo del tuo sconto di benvenuto.

Nei giorni scorsi ti ho spiegato perché capelli e unghie si indeboliscono, come funziona il collagene e perché servono dosaggi reali per vedere risultati.

Il codice **BENVENUTO** ti dà ancora **€10 di sconto sul primo acquisto**, valido su qualsiasi prodotto. Ma non resterà attivo per sempre.

Se non sai da dove partire:

**Per capelli, unghie e pelle** → **Youth** (6.000 mg collagene + 12 nutrienti, €1,97/giorno)

**Per il massimo dell'anti-aging cutaneo** → **Jeunesse** (12.000 mg collagene grass-fed + antiossidanti avanzati, €1,98/giorno)

**Per il protocollo completo** → Youth o Jeunesse + un multivitaminico. **[CTA: Scopri quale nella guida alla scelta](https://paleocomplex.com/guida-scelta)**

Oppure vai direttamente allo store: **[CTA: Usa il codice BENVENUTO](https://paleocomplex.com)**

Dopo questa email continuerai a ricevere contenuti di valore sulla salute e sull'integrazione. Niente spam, solo cose utili. Ma lo sconto di benvenuto non tornerà.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## Schema del flow

```
[Optin da landing social / form dedicati]
    │
    ▼
EMAIL 1 (immediata) — Delivery report + BENVENUTO €10
    │
    ├── Se acquista → esce → Post Purchase New (flow 6)
    │
    ▼  (+2 giorni)
EMAIL 2 — Perché capelli e unghie si indeboliscono (collagene, cofattori, antiossidanti)
    │
    ├── Se acquista → esce → Post Purchase New (flow 6)
    │
    ▼  (+4 giorni)
EMAIL 3 — Chi è Lorenzo + brand story (angolo beauty)
    │
    ├── Se acquista → esce → Post Purchase New (flow 6)
    │
    ▼  (+7 giorni)
EMAIL 4 — Youth e Jeunesse + social proof (il "costruttore")
    │
    ├── Se acquista → esce → Post Purchase New (flow 6)
    │
    ▼  (+9 giorni)
EMAIL 5 — Il multivitaminico come base (il "terreno")
    │
    ├── Se acquista → esce → Post Purchase New (flow 6)
    │
    ▼  (+12 giorni)
SPLIT: Ha acquistato?
    ├── SÌ → Fine (è nel Post Purchase)
    │
    └── NO ↓
         EMAIL 6 — Ultima occasione + reminder BENVENUTO
              │
              ▼
         Fine welcome → Entra in Authority Flow (flow 4)
```

---

## Note

### Recensioni utilizzate
Tutte verificate dal file `context/2026 03 31 paleocoplex export-reviews.csv`:
- **Email 4:** Manuela (Youth, 5 stelle, pelle + capelli in 10 giorni), Alice B. (Jeunesse, 5 stelle, pelle elastica + capelli + ciglia), Anna (Jeunesse, 5 stelle, unghie + capelli)
- **Email 5:** Cinzia G. (Paleocomplex, 5 stelle, caduta capelli risolta)

### Dati prodotto da verificare
- Youth: 6.000 mg collagene, €59/30 giorni → da `context/products/youth-scheda-prodotto.md`
- Jeunesse: 12.000 mg collagene grass-fed, €99/50 giorni → da `context/products/jeunesse-scheda-prodotto.md`
- Timing assunzione serale (glicina + rigenerazione notturna) → da intervista Lorenzo 01/04/2026

### Link da verificare prima della pubblicazione
- Link al PDF del report unghie/capelli → verificare URL attivo
- `paleocomplex.com/guida-scelta` → pagina da creare (vedi task-operativo)
- Codice BENVENUTO → verificare che sia attivo
- URL prodotto Youth e Jeunesse → verificare che siano attivi (Jeunesse potrebbe essere esaurito)
