**Versione:** 0.1
**Ultimo aggiornamento:** 2026-05-13

# Flow 21: Prodotto — Vitamina D in gocce

## Chi entra in questo flow

Persone che hanno acquistato la **Vitamina D in gocce** per la prima volta in assoluto.

## Configurazione Klaviyo

**Trigger:** Placed Order

**Filtro trigger (entrambe le condizioni):**
1. **Item SKU equals** `vitamina-d`
2. **Placed Order metric** → has happened **zero times** where Item SKU equals `vitamina-d` **over all time before this event**

## Mittenti

| # | Email | Mittente |
|---|-------|----------|
| 1 | Istruzioni | Lorenzo Zarone |
| 2 | Aspettative | Lorenzo Zarone |
| 3 | Social proof + check-in | Flaminia (Customer Care) |

## Timeline

| # | Timing | Tema | Tipo |
|---|--------|------|------|
| 1 | +1gg | Istruzioni assunzione Vitamina D | Statica |
| 2 | +5gg | Aspettative supplementazione vit D | Statica |
| 3 | +18gg | Social proof + check-in | Statica |

---

## EMAIL 1 — Istruzioni assunzione (+1 giorno)

**Mittente:** Lorenzo Zarone
**Tipo:** Statica

### Oggetto (3 varianti A/B)

- A: La tua Vitamina D è in viaggio. Ecco come usarla bene.
- B: Le istruzioni per partire con il piede giusto
- C: Cinque minuti per fare le cose nel modo corretto

### Preview text (3 varianti)

- A: Dose standard e dose personalizzata. Te lo spiego.
- B: Quante gocce, quando, come.
- C: Non è complicato. Ma c'è un modo giusto.

### Corpo email

Ciao [NOME]

La tua Vitamina D in gocce è in viaggio (se non te l'ha già consegnato il corriere mentre leggi). Prima che la apri voglio assicurarmi che parti con il piede giusto.

**Dosaggio**

La tua Vitamina D è formulata in olio MCT (un grasso): può essere assunta anche a stomaco vuoto, non richiede il pasto.

Dose standard: 4 gocce al giorno a colazione, equivalenti a circa 2.000 UI. Ogni goccia contiene circa 500 UI. Il dosaggio può essere modulato in base ai tuoi livelli ematici:

- 1-2 gocce al giorno: dose minima di mantenimento
- 4 gocce: dose standard per la maggior parte degli adulti
- 8 gocce: protocollo intensivo per carenze marcate (consultare il medico)

**Se non conosci i tuoi livelli**

Parti con 4 gocce al giorno per 3 mesi e poi fai un controllo della 25(OH)D. In base al risultato puoi regolare il dosaggio per arrivare al range ottimale (50-80 ng/ml).

**Per dubbi specifici**

Se hai dubbi sul dosaggio o vuoi adattare l'assunzione a una tua condizione (gravidanza, allattamento, patologie, farmaci, soprattutto se assumi anticoagulanti coumarinici), contattaci dalla **[pagina contatti del sito](https://paleocomplex.com/contatti/)**. Per qualunque altra domanda sui prodotti, la pagina **[FAQ del sito](https://paleocomplex.com/faq/)** copre quasi tutti i casi.

**Tra qualche giorno** ti dico cosa aspettarti realisticamente.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 2 — Aspettative (+5 giorni)

**Mittente:** Lorenzo Zarone
**Tipo:** Statica

### Oggetto (3 varianti A/B)

- A: Cosa aspettarti dalla tua Vitamina D
- B: Vitamina D: l'unico integratore davvero misurabile
- C: I tempi veri della Vitamina D

### Preview text (3 varianti)

- A: La vit D è uno dei pochi integratori che si misura con un'analisi.
- B: Te lo spiego con onestà.
- C: Quando senti cosa, e perché.

### Corpo email

Ciao [NOME]

A questo punto hai (spero) iniziato la tua Vitamina D da qualche giorno. È il momento giusto per parlarti di una cosa importante: cosa aspettarti realisticamente.

**Dopo 2-4 settimane**

I livelli ematici di 25(OH)D iniziano a salire. Effetti percepibili su umore ed energia, se la carenza era significativa.

**Dopo 2-3 mesi**

I livelli si stabilizzano. Il sistema immunitario è più reattivo. Le ossa beneficiano dell'aumentato assorbimento di calcio. Il miglioramento è documentabile con un'analisi del sangue (25(OH)D).

**Uso continuativo**

La vitamina D è una supplementazione che ha senso a vita per chi vive in Italia, specialmente da ottobre a marzo quando il sole non basta a stimolare la produzione cutanea. Mantenere il 25(OH)D tra 50 e 80 ng/ml richiede integrazione costante: controlla i livelli a fine marzo e fine settembre per regolare il dosaggio.

**Cosa rende la Vitamina D diversa**

A differenza di quasi tutti gli altri integratori, la vitamina D ha un effetto **misurabile in modo oggettivo**: basta un'analisi del sangue (25(OH)D, costa pochi euro) per sapere esattamente a che livello sei. È l'unico integratore in cui non devi affidarti alle "sensazioni" per sapere se sta funzionando.

Il mio consiglio: se non l'hai mai fatto, controlla i tuoi livelli adesso (prima di vedere l'effetto della supplementazione), e poi ricontrolla tra 3 mesi. La differenza ti dirà esattamente quanto la tua dose è giusta o va modificata.

**Più avanti** Flaminia, una persona del nostro team, ti scriverà per sentire come stai andando.

Un forte abbraccio
Lorenzo Zarone
Fondatore di Paleocomplex

---

## EMAIL 3 — Social proof + check-in (+18 giorni)

**Mittente:** Flaminia (Customer Care)
**Tipo:** Statica

Recensioni verificate dal CSV `context/20260511 paleocomplex export-reviews.csv` — filtro `product_sku = vitamina-d` (4 recensioni 5 stelle disponibili, brevi). Selezionate 2 top: artrite reumatoide (Stefano), fiducia generica (Cinzia).

### Oggetto (3 varianti A/B)

- A: Come sta andando con la tua Vitamina D?
- B: Sono passate quasi tre settimane
- C: Volevo sentirti

### Preview text (3 varianti)

- A: Sono Flaminia, faccio parte del team di Lorenzo. Mi farebbe piacere sapere come va.
- B: La vit D si misura. Te lo ricordo.
- C: Ho raccolto qualche esperienza che potrebbe interessarti.

### Corpo email

Ciao [NOME]

Ti scrivo io questa volta. Sono Flaminia e mi occupo del rapporto con i clienti di Paleocomplex. Lorenzo si occupa della parte tecnica e scientifica, io invece sono il punto di contatto umano: rispondo a chi ci scrive, ascolto i feedback, raccolgo storie. Ed è proprio per questo che oggi voglio scriverti.

Sono passate circa 2-3 settimane da quando hai iniziato la tua Vitamina D. Su questo prodotto in particolare, voglio ricordarti una cosa importante che ti aveva detto Lorenzo: la vit D è uno dei pochi integratori con effetti misurabili in modo oggettivo, basta un'analisi della 25(OH)D.

Le esperienze di chi la usa parlano chiaro:

*"Ne prendo sempre 4 gocce al mattino e i risultati si vedono, specialmente con la mia patologia di artrite reumatoide."*
**Stefano O.**

*"Ho piena fiducia nei prodotti proposti da Paleocomplex, non ho bisogno di essere convinta sull'assunzione della Vitamina D in gocce. Consigliata a tutti."*
**Cinzia G.**

I risultati sono personali. Ma a differenza di quasi tutti gli altri integratori, la Vitamina D si misura: per capire se il dosaggio è quello giusto per te, controlla i livelli dopo 3 mesi e regola di conseguenza.

**Una nota pratica**: il range ottimale di 25(OH)D è 50-80 ng/ml. Sotto 30 ng/ml è insufficienza, sotto 20 è carenza. Sopra 100 ng/ml è eccesso (raro con dosi standard).

E se hai già fatto l'analisi o stai per farla, mi farebbe davvero piacere saperlo. **Rispondi pure a questa email** raccontandomi i tuoi valori (anche solo prima/dopo): per noi è prezioso costruire un quadro reale di come la nostra Vitamina D funziona per persone diverse.

Flaminia
Customer Care Paleocomplex

---

### Status
Bozza v0.1 — in attesa review.
