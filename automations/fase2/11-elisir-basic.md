...
Persone che hanno acquistato **Elisir Basic** per la prima volta in assoluto.
...
1. **Item SKU equals** `elisir-basic`
2. **Placed Order metric** → has happened **zero times** where Item SKU equals `elisir-basic` **over all time before this event**
...
| # | Timing | Tema | Tipo |
|---|--------|------|------|
| 1 | +1gg | Istruzioni assunzione Elisir Basic | Statica |
...
- A: Il tuo Elisir Basic è in viaggio. Ecco come usarlo bene.
...
Sciogli la polvere in un bicchiere di acqua fresca o a temperatura ambiente. Mai calda: le vitamine idrosolubili e altri ingredienti si ossidano. Bevi dopo colazione o dopo pranzo, mai a stomaco vuoto: le vitamine liposolubili A, E, K hanno bisogno dei grassi del pasto per essere assorbite meglio.
...
- A: Cosa aspettarti realisticamente dal tuo Elisir Basic
...
I primi segnali sono sottili ma reali. Il magnesio può migliorare la qualità del sonno e ridurre i crampi notturni già nei primi giorni. Le vitamine B attivate iniziano a sostenere il metabolismo energetico. Chi aveva carenze importanti può notare una maggiore stabilità dell'umore.
...
È il momento in cui controllare le analisi del sangue ha senso. Ferritina, omocisteina, profilo vitaminico generale (esclusa vit D che con Elisir Basic gestisci a parte). La pelle è più luminosa. Il sistema immunitario è più reattivo. Chi smette nota subito: "mi manca qualcosa".
...
Recensioni verificate dal CSV `context/20260511 paleocomplex export-reviews.csv` — filtro `product_sku = elisir-basic` (12 recensioni 5 stelle disponibili). Selezionate 3 top: uso famigliare cross-generazione (Massimo), energia generale (Federica), benessere over 60 (Flavio).

### Oggetto (3 varianti A/B)

- A: Come sta andando con il tuo Elisir Basic?
...
*"È un prodotto eccezionale. Mi dà energia ed ha tutto ciò di cui ho bisogno al suo interno. Sono super soddisfatta. Lo consiglio perché in una sola soluzione prendi tutto."*
**Federica Bracaglia**
...
I risultati sono personali, dipendono dalle condizioni di partenza e dallo stile di vita. Una nota specifica per chi prende Elisir Basic: essendo la versione senza vitamina D, ricordati di gestirla separatamente. Se non l'hai già nel protocollo, le nostre **[gocce di Vitamina D](https://paleocomplex.com/prodotto/vitamina-d/)** sono pensate esattamente per questo: 4 gocce al giorno coprono la maggior parte dei casi.
