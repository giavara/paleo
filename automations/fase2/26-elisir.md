...
# Flow 26: Prodotto — Elisir

## Chi entra in questo flow

Persone che hanno acquistato **Elisir** (versione completa, non Basic) per la prima volta in assoluto.
...
1. **Item SKU equals** `elisir`
2. **Placed Order metric** → has happened **zero times** where Item SKU equals `elisir` **over all time before this event**
...
| # | Timing | Tema | Tipo |
|---|--------|------|------|
| 1 | +1gg | Istruzioni assunzione Elisir | Statica |
| 2 | +5gg | Aspettative multivitaminico completo | Statica |
...
- A: Il tuo Elisir è in viaggio. Ecco come usarlo bene.
...
- A: Cosa aspettarti realisticamente dal tuo Elisir
...
Recensioni verificate dal CSV `context/20260511 paleocomplex export-reviews.csv` — filtro `product_sku = elisir` (22 recensioni 5 stelle disponibili). Selezionate 3 top: completezza nutrizionale (Anceschi), vitalità + concentrazione (Lapresa), supporto attività fisica intensa (Buti).

### Oggetto (3 varianti A/B)

- A: Come sta andando con il tuo Elisir?
...
*"Integratore ottimo, l'ho utilizzato con costanza e ho visto risultati concreti, soprattutto in termini di vitalità e concentrazione. Consigliatissimo."*
**Paola Lapresa**

*"Un prodotto che ritengo importante in periodi molto impegnativi della vita. Riesco a sopperire alle attività quotidiane pur avendo un'attività fisica intensa da gestire."*
**Marco Buti**

I risultati sono personali, dipendono dalle condizioni di partenza e dallo stile di vita. Se col solo Elisir non raggiungi livelli ottimali di vitamina D (sopra i 40 ng/ml), il consiglio non è raddoppiare la dose ma aggiungere le **[gocce di Vitamina D](https://paleocomplex.com/prodotto/vitamina-d/)**: 4 gocce al giorno coprono la maggior parte dei casi.
